...
static void shiftviewclientsandmove(const Arg *arg);
...
void
shiftviewclientsandmove(const Arg *arg)
{
	Arg shifted;
	Client *c;
	unsigned int tagmask = 0;
...
{
	selmon->showbar = (selmon->pertag->showbars[selmon->pertag->curtag] == 2 ? 1 : !selmon->showbar);
	updatebarpos(selmon);
	resizebarwin(selmon);
	if (showsystray) {
		XWindowChanges wc;
